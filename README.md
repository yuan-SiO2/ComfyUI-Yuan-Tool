...
| **H3 运动裁剪**         | 按「衔接模式」对 H3 AV 潜空间两段式裁切：视频图像模式解码为图像+音频后按像素裁切、尾段存 mp4，潜空间模式按整 VRF 组直接裁切、尾段存 safetensors；输出槽随模式切换（图像+音频 / 潜空间），尾部按「保存到本地」保存一段供下一片段衔接 |
...
- **2026-09-18**：「H3 运动上下文」「H3 运动裁剪」新增「衔接模式」开关，兼顾潜空间与视频图像两条衔接路径；端口、参数与输出槽随模式切换（裁剪节点输出 图像+音频 或 潜空间）。
